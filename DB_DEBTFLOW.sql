...
-- Agregar índice único si no existe (para PostgreSQL)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'people_owner_id_name_key'
  ) THEN
    ALTER TABLE people ADD CONSTRAINT people_owner_id_name_key UNIQUE (owner_id, name);
  END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS incomes (
  id              uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  owner_id        uuid NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  source          text NOT NULL CHECK (source IN ('salary', 'freelance', 'investment', 'business', 'gift', 'other')),
...
CREATE INDEX IF NOT EXISTS idx_incomes_owner ON incomes(owner_id);
...
CREATE POLICY "incomes_insert" ON incomes FOR INSERT WITH CHECK (true);
DROP POLICY IF EXISTS "incomes_update" ON incomes;
CREATE POLICY "incomes_update" ON incomes FOR UPDATE USING (true);
DROP POLICY IF EXISTS "incomes_delete" ON incomes;
CREATE POLICY "incomes_delete" ON incomes FOR DELETE USING (true);
...
INSERT INTO system_settings (key, value, description, category) VALUES
  ('company_name', 'DebtFlow Pro', 'Nombre de la empresa', 'general'),
  ('currency', 'USD', 'Moneda por defecto', 'general'),
  ('timezone', 'America/Bogota', 'Zona horaria', 'general'),
  ('email_overdue', 'true', 'Notificar deudas vencidas', 'notifications'),
  ('email_reminder', 'true', 'Recordatorios de pago', 'notifications'),
  ('email_new_user', 'false', 'Notificar nuevos usuarios', 'notifications'),
  ('min_password_length', '8', 'Longitud mínima de contraseña', 'security'),
  ('session_timeout', '30', 'Tiempo de sesión en minutos', 'security'),
  ('two_factor_enabled', 'false', 'Habilitar 2FA', 'security'),
  ('webhook_url', '', 'URL del webhook', 'integrations')
